...
async function main() {
  ...
  try {
    ...
    // Use bash to handle eval and ssh-agent properly
    const sshAgentOutput = await $`bash -c 'ssh-agent -s'`.text();
    const sshAuthSockMatch = sshAgentOutput.match(/SSH_AUTH_SOCK=([^;]+)/);
    const sshAgentPidMatch = sshAgentOutput.match(/SSH_AGENT_PID=(\d+)/);
    ...
    // Add SSH key using echo to avoid heredoc issues
    await $`bash -c "echo '${process.env.DEPLOY_KEY_GITHUB_IO}' | ssh-add -"`;
    ...
    const diffResult = await $`git diff --staged --quiet`.catch(
      () => "changes"
    );

    if (diffResult === "changes") {
      const commitMessage = `deploy: update from fossiq/root@${
