...
console.log(
  "   If you see errors, install Emscripten or use a prebuilt WASM from CI."
);

try {
  await $`bun x tree-sitter build --wasm --js-runtime bun`;
  ...
  console.error(
    "  3. Or, copy a prebuilt tree-sitter-kql.wasm from a recent release"
  );
  process.exit(1);
}
