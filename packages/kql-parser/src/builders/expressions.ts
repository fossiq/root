...
export function buildComparisonExpression(
  node: SyntaxNode,
  buildAST: (node: SyntaxNode) => ASTNode
): ComparisonExpression {
  const left = node.child(0);
  ...
  for (let i = 0; i < node.childCount; i++) {
    ...
    if (
      ...
      (child.type === "expression" ||
        child.type.includes("_expression") ||
...
export function buildNamedArgument(
  node: SyntaxNode,
  buildAST: (node: SyntaxNode) => ASTNode
): NamedArgument {
  const nameNode = node.child(0);
...
export function buildTypeCastExpression(
  node: SyntaxNode,
  buildAST: (node: SyntaxNode) => ASTNode
): TypeCastExpression {
...
export function buildBetweenExpression(
  node: SyntaxNode,
  buildAST: (node: SyntaxNode) => ASTNode
): BetweenExpression {
  const left = node.child(0);
  // between_value nodes contain either a literal or a function_call
  const betweenValues = node.children.filter((c) => c.type === "between_value");
  ...
  if (betweenValues.length !== 2) {
    throw new Error("Between expression requires exactly two values");
  }

  // Extract the actual value from each between_value wrapper
  const extractValue = (betweenValue: SyntaxNode): ASTNode => {
    const child = betweenValue.child(0);
    if (!child) {
      throw new Error("Between value is empty");
    }
  ...
  return {
    ...
    left: buildIdentifier(left),
    min: extractValue(betweenValues[0]) as Expression,
    max: extractValue(betweenValues[1]) as Expression,
  };
}
