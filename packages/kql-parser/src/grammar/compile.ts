...
function serializeRule(rule: Rule, indent: string = '  '): string {
  ...
  switch (rule.type) {
    ...
    case 'SEQ':
      return `seq(${rule.members.map(r => serializeRule(r, indent)).join(', ')})`;

    case 'CHOICE':
      if (rule.members && rule.members.length === 2 && rule.members[1].type === 'BLANK') {
        // This is an optional
        return `optional(${serializeRule(rule.members[0], indent)})`;
      }
      return `choice(${rule.members?.map(r => serializeRule(r, indent)).join(', ')})`;
    ...
    case 'PREC_RIGHT':
      ...
      return `prec.right(${rule.value}, ${serializeRule(rule.content, indent)})`;

    case 'PREC_DYNAMIC':
      return `prec.dynamic(${rule.value}, ${serializeRule(rule.content, indent)})`;

    case 'TOKEN':
      return `token(${serializeRule(rule.content, indent)})`;

    case 'ALIAS':
      return `alias(${serializeRule(rule.content, indent)}, ${JSON.stringify(rule.value)})`;

    case 'FIELD':
      return `field(${JSON.stringify(rule.name)}, ${serializeRule(rule.content, indent)})`;
