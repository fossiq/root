...
export function mapParseOperator(
  ...
  if (!exprNode || !patternNode) return ctx.errorNode(node, "Parse missing expression or pattern");
        ...
        for (const lit of literals) {
            ...
            if (valNode) {
                 if (valNode.type.name === "Number") {
                     val = { type: "NumberLiteral", value: parseFloat(ctx.slice(valNode)), raw: ctx.slice(valNode), start: valNode.from, end: valNode.to };
                 } else if (valNode.type.name === "String") {
                     val = { type: "StringLiteral", value: ctx.parseStringLiteral(ctx.slice(valNode)), raw: ctx.slice(valNode), start: valNode.from, end: valNode.to };
                 } else {
                     // Boolean or null keywords
                     const raw = ctx.slice(valNode);
                     val = { type: "Literal", value: raw === "null" ? null : raw === "true", raw, start: valNode.from, end: valNode.to };
                 }
                 currentRow.push(val);
            }
